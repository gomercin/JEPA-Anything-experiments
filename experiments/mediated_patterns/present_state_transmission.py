...
OLD = Path(
    "evidence/history-conditioned-transmission-2026-09-26/data/work/mediated_patterns/history_conditioned_transmission"
)
...
def paired_indices(rows):
    ...
    for j, row in enumerate(rows):
        ...
        candidates = [
            ...
            if other["seed"] == row["seed"]
            and other["wait"] == row["wait"]
            and other["history"] == "none"
        ]
        if len(candidates) != 1:
    ...
    for seed in DEV:
        ...
        for wait in WAITS:
            for index, history in enumerate(["none", "odd04"]):
                ...
                target = (
                    root
                    / f"s{seed}-t{wait:g}-{'written' if index else 'unwritten'}.npz"
                )
                ...
                rows.append({"seed": seed, "wait": wait, "history": history})
    return np.asarray(z), np.asarray(y), rows, inputs


def old_initial(seed):
    path = OLD / (
        "pilot-01/preparation.npz"
        if seed == 8101
        else f"fresh-01/s{seed}-preparation.npz"
    )
    with np.load(path, allow_pickle=False) as z:
...
def reference(state, config, budget):
    ...
    return {
        "absolute": absolute,
        "response": absolute[:, 1, 2, :2] - absolute[:, 0, 2, :2],
        "time": model.TIMES,
    }, screens
...
def run_states(out, seeds, families, budget, fresh=False, frozen=None):
    ...
    for seed in seeds:
        ...
        budget.finish()
        save_npz_exclusive(
            out / f"s{seed}-history.npz", checkpoints=cp, checkpoint_times=WAITS
        )
        model.save_json(
            out / f"s{seed}-history.json", {"families": families, "regime": screens}
        )
        for wi, wait in enumerate(WAITS):
            for hi, history in enumerate(families):
                rows.append({"seed": seed, "wait": wait, "history": history})
                states.append(cp[wi, hi])
                extraction_start = time.process_time()
                descriptors.append(model.extract(cp[wi, hi]))
                extraction_cpu += time.process_time() - extraction_start
    ...
    if fresh:
        ...
        forecast_start = time.process_time()
        save_npz_exclusive(
            out / "predictions.npz",
            **{
                name: model.predict(m, descriptors[:, m["columns"]])
                for name, m in predictors.items()
            },
        )
        forecast_cpu = time.process_time() - forecast_start
        model.save_json(
            out / "inference-cost.json",
            {
                "panel_all_descriptor_extraction_cpu": extraction_cpu,
                "panel_all_model_forecast_and_save_cpu": forecast_cpu,
                "states": len(states),
                "current_grid_values_per_state": 2 * CFG.n,
                "extracted_panel_values_per_state": len(model.NAMES),
                "deployment_descriptor_count": {
                    k: len(v["columns"]) for k, v in predictors.items()
                },
                "models_json_bytes": (frozen / "models.json").stat().st_size,
            },
        )
        model.save_json(
    ...
    for i, (row, state) in enumerate(zip(rows, states, strict=True)):
        ...
        budget.finish()
        print(
            json.dumps({"reference": row, "cpu": budget.sections[-1]["cpu_seconds"]}),
            flush=True,
        )
...
def fit_panel(out, extension, budget, pair_weight=10.0):
    ...
    for feature_set in model.SETS:
        options = (
            [(1, 1e-6)]
            if feature_set == "blind"
            else [(d, r) for d in (1, 2) for r in (1e-6, 1e-3)]
        )
    ...
    best = {
        s: min(
            (c for c in candidates if c["feature_set"] == s),
    ...
    summary = {
        ...
        "best": best,
        "inputs": inputs,
        "preparations": sorted(set(groups)),
        "rows": rows,
        "feature_min": z.min(axis=0).tolist(),
        "feature_max": z.max(axis=0).tolist(),
        "feature_correlation": np.corrcoef(z.T).tolist(),
        "best_objectives": {
            s: next(c["objective"] for c in candidates if c["name"] == n)
    ...
    print(
        json.dumps({"best": best, "objectives": summary["best_objectives"]}), flush=True
    )
...
def refine(out, data, seed, history, budget, fresh=False):
    ...
    for suffix, config in [
        ...
        budget.finish()
        save_npz_exclusive(
            out / f"{suffix}-checkpoints.npz", checkpoints=cp, checkpoint_times=WAITS
        )
        ...
        for wi, wait in enumerate(WAITS):
            ...
            for hi, name in enumerate(["none", history]):
                ...
                index = next(
                    ...
                    if r == {"seed": seed, "wait": wait, "history": name}
                )
                diff = actual["response"] - base_targets[index]
                errors.append(
                    [
                        np.sqrt(np.mean(diff[mask] ** 2, axis=0))
                        for mask in [model.TIMES >= 0, model.TIMES >= 40]
                    ]
                )
                budget.finish()
            results.append(
                {
                    "refinement": suffix,
                    "wait": wait,
                    "branch_errors": np.asarray(errors).tolist(),
    ...
    model.save_json(
        out / "refinement.json",
        {
            ...
            "checks": results,
            "floors": np.maximum(
                1e-12, 5 * np.max([r["sum_errors"] for r in results], axis=0)
            ).tolist(),
        },
    )
...
def main():
    ...
    budget = HistoryBudget(
        previous, 1800 if args.stage == "fresh" or args.fresh_refinement else 1200
    )
        ...
        model.save_json(
            args.output / "protocol.json",
            {
                ...
                "report_sha256": digest(
                    Path(__file__).with_name("PRESENT_STATE_TRANSMISSION.md")
                ),
                "previous_cpu": previous,
                "seed_split": {"development": DEV, "fresh": FRESH},
            },
        )
        budget.check()
        if args.stage == "fit":
            ...
            run_states(
                args.output,
                [8101, 10101],
                {"negative": [-0.4, 0], "mixed": [0.28, 0.28]},
                budget,
            )
            ...
            run_states(
                args.output, FRESH, contract["families"], budget, True, args.freeze
            )
        else:
            refine(
                args.output,
